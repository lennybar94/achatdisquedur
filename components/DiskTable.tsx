...
interface DiskTableProps {
  selectedKind: 'HDD/SSD' | 'Disque Flash' | 'Carte Mémoire';
}

export default function DiskTable({ selectedKind }: DiskTableProps) {
  ...
  // Chargement des données en fonction du selectedKind passé en prop
  useEffect(() => {
    let source: Disk[];
  ...
  const { brands, storageTypes, interfaces } = useMemo(() => {
    const b = new Set<string>(), s = new Set<string>(), iface = new Set<string>();
  ...
  return (
    <Fragment>
      <Head><title>Meilleures ventes de {selectedKind} 2025</title></Head>
      ...
      {showFilters && (
        <aside className="filter-panel">
          ...
          <div className="filter-box">
            <h3>Marque</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '.25rem .75rem' }}>
              {brands.map(b => (
                <label key={b}>
                  <input
                    type="checkbox"
          ...
          {/* Type de filtre conditionnel */}
          <div className="filter-box">
            <h3>{selectedKind === 'HDD/SSD' ? 'Type de stockage' : 'Interface'}</h3>
            {(selectedKind === 'HDD/SSD' ? storageTypes : interfaces).map(item => (
              <label key={item}>
                <input
                  type="checkbox"
                  checked={selectedKind === 'HDD/SSD' ? selectedStorage.has(item) : selectedInterface.has(item)}
                  onChange={e => {
                    const c = new Set<string>(selectedKind === 'HDD/SSD' ? selectedStorage : selectedInterface);
                    e.target.checked ? c.add(item) : c.delete(item);
                    if (selectedKind === 'HDD/SSD') setSelectedStorage(c);
                    else setSelectedInterface(c);
                    setCurrentPage(1);
                  }}
                /> {item}
              </label>
              ...
              return (
                <tr key={d.asin}>
                  ...
                  <td style={{ textAlign: 'left' }}>
                    <a href={d.url_affiliate} target="_blank" rel="nofollow sponsored">
                      {d.title}
                    </a>
                  </td>
